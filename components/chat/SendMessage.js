    ...
    const sendMessage = async (event) => {
        ...
        if (Array.isArray(participants)) {
            participants.forEach((participant) => {
                if (participant.userAddress !== undefined && participant.userAddress !== null)
                    addNotification(participant.userAddress, `You have received a new message from ${account}`, account, itemId, `chats/${chatID}`, "chat")
            })
        }
        // for chat popup component participants have structure {seller:"0x...", buyer:"0x..", "moderator":"0x.."}
            ...
            if (participants.seller !== "" && participants.seller !== account)
                await addNotification(participants.seller, `You have received a new message from ${account}`, account, itemId, `chats/${chatID}`, "chat")
            if (participants.buyer !== "" && participants.buyer !== account)
                await addNotification(participants.buyer, `You have received a new message from ${account}`, account, itemId, `chats/${chatID}`, "chat")
            if (participants.moderator !== "" && participants.moderator !== account)
                await addNotification(participants.moderator, `You have received a new message from ${account}`, account, itemId, `chats/${chatID}`, "chat")
        }
    };
